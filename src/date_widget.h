...
public:
    ...
    static void update_date(tm date) {
        ...
        char buffer[15];
        sprintf(buffer, "%d/%d/%d", date.tm_mday, date.tm_mon + 1, date.tm_year + 1900);
    ...
    void update() {
        ...
        debug("today: %d/%d/%d, last_update: %d/%d/%d", today.tm_mday, today.tm_mon + 1, today.tm_year + 1900, last_update.tm_mday, last_update.tm_mon + 1, last_update.tm_year + 1900);
        last_update = today;
